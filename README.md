<img src="https://raw.githubusercontent.com/salman-bermano/bulk-downloader/refs/heads/main/picture.png" width="621">

# bulk downloader
download files in bulk.

## download
if you don't want to use the downloader.py script with python, you can use the compiled c# version of the script, c#-downloader.exe<br>
you can download it [here](https://github.com/salman-bermano/bulk-downloader/releases/latest)

## how to use
simply run the python script, paste the text file containing the download links, and the script will download all the files. the downloaded files will be stored in the folder named "bulk_downloads" 

## note
you MUST have curl.exe installed on your computer or have curl.exe sit right next to this python script in order for this script to work
